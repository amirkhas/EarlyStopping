...
def filter_points(ang, mag, percent_to_keep):

    ### angle the lower the better
    ### mag the higher the better
    score_len = len(ang)
    
    num_to_pick = int(percent_to_keep * score_len)

    bottom_10_ang = heapq.nsmallest(num_to_pick, range(score_len), key=lambda i: ang[i])
    top_10_mag = heapq.nlargest(num_to_pick, range(score_len), key=lambda i:  mag[i])

    combined_indices = sorted(list(set(bottom_10_ang + top_10_mag)))
...
def plot_labels(feat1, feat2, labels, model_name, idx, reward_list):
...
# with open(f"math500_outputs_{model_name}_sk.pkl", "rb") as f:
...
for i in range(num_sample):
    ...
    feat_list_1, feat_list_2, feat_list_3, feat_list_4, binary_list, reward_list = [], [], [], [], [], []
    ...
    plot_labels(feat_list_1, feat_list_2, binary_list, model_name, i, reward_list)
    # if i == 108:
        # plot_labels_3d_interactive(feat_list, feat_list_2, feat_list_4, binary_list, model_name, i, reward_list)
    ...
    print(f"Processed {len(processed)} scores out of {N}")
